...
- **Force `NODE_ENV=production` in the build script** of every Next.js project on this machine. It costs nothing and remains good defense-in-depth even now that the global export is gone. Root cause resolved: the `export NODE_ENV=development` line was **removed from `zsh/zshenv` on 2026-06-13** (flagged 2026-06-11). With `NODE_ENV` unset, every tool falls back to its own correct default (`next dev` → development, `next build` → production), so there is no longer a machine-global value to leak into builds.
